...
function App() {
  ...
  const defenseThreats = useMemo(
    () =>
      party.length >= 2 ? analyzeDefenseThreatPokemon(party, opponentPool) : [],
    [party, opponentPool]
  );
        ...
        <div className="partyGrid movePartyGrid compactPartyGrid">
          {partySlots.map((slot, index) => (
            <PartySlotCard
              ...
              onStartMoveEdit={(moveSlotIndex) =>
                startMoveEdit(index, moveSlotIndex)
              }
              onCancelMoveEdit={cancelMoveEdit}
              onMoveQueryChange={updateMoveQuery}
              onSelectMove={addMoveToSlot}
...
function analyzeOffenseSingleTypesFromSlots(
  ...
  const results = POKEMON_TYPES.map((defenseType) => {
    const details = activeSlots.map((slot) =>
      getBestOffenseDetailForSlot(slot, [defenseType])
    );
...
function analyzeDefenseThreatPokemon(
  ...
  const results = opponents.map((opponent) => {
    ...
    const details = party.map((targetPokemon) => {
      const candidates = attackOptions.map((attackOption) => {
        const multiplier = getFinalDefenseMultiplier({
          attackType: attackOption.type,
          defenseTypes: targetPokemon.types,
          defensiveAbilityEffect: targetPokemon.defensiveAbilityEffect,
        });
...
function analyzeOffenseProblemPokemon(
  ...
  const results = opponents.map((opponent) => {
    const details = activeSlots.map((slot) =>
      getBestOffenseDetailForSlot(slot, opponent.types)
    );
...
function getBestOffenseDetailForSlot(
  ...
  const candidates = attackOptions.map((attackOption) => {
    const multiplier = getTypeMultiplier(attackOption.type, defenseTypes);
